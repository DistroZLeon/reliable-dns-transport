    ...
    while(True):
        ...
        if qtype== "NS" and qname== domain:
            ...
            if seq_num== 0:
                ...
                client_pub= raw_data[:65]
                client_hmac= raw_data[65: 97]

                if not hsm.verify_hmac(client_pub, client_hmac):
                ...
                server_hmac= hsm.gen_hmac(server_pub)

                rdata= encode_txt(server_pub+ server_hmac)
                ...
                if action== "download" and seq_num> 1:
                    ...
                    if chunk_idx<= total_chunks:
                        ...
                        ack_packet= Packet(ack_num= seq_num, flags= rflags, data= file_data)

                    else:
                        ack_packet= Packet(ack_num= seq_num, flags= Packet.FIN)

                else:
                    ack_packet= Packet(ack_num= seq_num, flags= Packet.ACK)
